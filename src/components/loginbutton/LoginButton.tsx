...
const LoginButton = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (accessToken) {
        getUserGraphDetails(accessToken);
  ...
  const getUserGraphDetails = async (accessToken:string) => {
    try {
      const urlData = 'https://graph.microsoft.com/v1.0/me/getMemberObjects';
      ...
      console.log("Success graph:", responseData);
      
      if (response.ok) {
        getUserGraphIdDetails(responseData.value, accessToken);
      }
  ...
  const getUserGraphIdDetails = async (groupIds:Array<string>, accessToken:string) => {
    try {
      const urlData = 'https://graph.microsoft.com/v1.0/directoryObjects/getByIds';
      ...
      const responseData = await response.json();
      console.log("Success graph id:", responseData);

      if (response.ok) {
        const group_names = responseData.value.map((g: any) => g.displayName);
        console.log("Group Names:", group_names);
        const filteredGroups = group_names.filter(
          (group: string) => group.toUpperCase().includes('OPTIMUS') || group.toUpperCase().includes('CIRRUS_GENAI')
        );
        console.log("Filtered Groups>:", filteredGroups);

        if (filteredGroups.length > 0) {
          const isUser = filteredGroups.some((group: string) => group.toUpperCase() === 'HID100007708_PROD_CIRRUS_GENAI_OPTIMUS_USER');
          const isAdmin = filteredGroups.some((group: string) => group.toUpperCase() === 'HID100007708_PROD_CIRRUS_GENAI_OPTIMUS_ADMIN');
          const userData = {
            verification: true,
            roles: {
              user: isUser || true,
